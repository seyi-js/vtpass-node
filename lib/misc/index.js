...
module.exports = class Misc {
     ...
     * @returns {JSON}
     */
    async getBankVariationCodes(serviceID='bank-deposit') {
        try {
            const response = await this.axios.get(`/service-variations?serviceID=${serviceID}`);
     ...
     * @returns 
     */
    async verifyAccountNumber(bank_name, account_number, serviceID='bank-deposit') {
        try {
            const response = await this.axios.post('/merchant-verify',{serviceID,type:bank_name,accountNumber:account_number});
    ...
    async queryTransaction(request_id){
        try {
            const response = await this.axios.post('/requery',{request_id});
